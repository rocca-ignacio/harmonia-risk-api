...
# Run tests (use venv directly — pytest/python are not on PATH)
.venv/bin/pytest tests/ -v

# Run a specific test
.venv/bin/pytest tests/test_api.py::test_function_name
```

`data/test_scenarios.json` is auto-generated by `seed_data` — timestamp drift on each run is expected, not a bug.
...
**Blocklist/allowlist caching**: `cachetools.TTLCache` (maxsize=10000) keyed by `(entry_type, value, merchant_id)`, cleared on any mutation. Rules cache uses `TTLCache(maxsize=1000)`.

**Signal timeout**: Each signal is wrapped in `asyncio.wait_for(timeout=SIGNAL_TIMEOUT_SECONDS)` in `risk_engine.py`. Timed-out signals return score 0 and processing continues.

**Analytics**: `app/routers/analytics.py` exposes `GET /api/v1/analytics/{summary,signals,trends}` — queries the `risk_audit` table.
...
| `HARMONIA_SIGNAL_TIMEOUT_SECONDS` | `2.0` | Per-signal async timeout (seconds) |
